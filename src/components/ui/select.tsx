...
function extractItemsFromChildren(children: React.ReactNode): Record<string, React.ReactNode> {
  ...
  React.Children.forEach(children, (child) => {
    if (React.isValidElement(child)) {
      if (child.type === SelectItem || (child.props as any)?.["data-slot"] === "select-item") {
        const value = (child.props as any).value
        const childChildren = (child.props as any).children
        if (value !== undefined) {
          items[value] = childChildren
        }
      }
      if (child.type === SelectContent) {
        const contentChildren = (child.props as any).children
        React.Children.forEach(contentChildren, (innerChild) => {
          if (React.isValidElement(innerChild)) {
            const value = (innerChild.props as any).value
            const innerChildren = (innerChild.props as any).children
            if (value !== undefined) {
              items[value] = innerChildren
            }
          }
        })
      }
...
interface SelectWithLabelsProps {
...
function SelectWithLabels({ children, items: itemsProp, ...props }: SelectWithLabelsProps) {
